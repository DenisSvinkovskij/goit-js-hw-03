...
const game = {
  ...
  fight() {
    ...
    while (firstPlayerHealth > 0 && secondPlayerHealth > 0) {
      ...
      if (secondPlayerHealth <= 0 && secondPlayerHealth < firstPlayerHealth) {
      ...
      if (firstPlayerHealth <= 0 && firstPlayerHealth < secondPlayerHealth) {
  ...
  logPlayersHit({ name, health, icon, damage }) {
    console.log(
      `Игрок ${icon}${name} 🤜💨(${damage}). У противника осталось %c${health}`,
      'color: red;',
    );
  },
...
const firstPlayer = playerFactory('Sindel');
